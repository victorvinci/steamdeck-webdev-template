...
3. **Attribution commit.** Append the JSONL line with the SHA from step 2, then commit just `.ai-attribution.jsonl` with a short message like `chore(attribution): log <scope> [skip ci]`. Same `Co-Authored-By` trailer applies. The `[skip ci]` marker is belt-and-braces — `.ai-attribution.jsonl` is also in `paths-ignore` for the push triggers in `ci.yml` and `codeql.yml`, so a standalone attribution-only push is skipped either way. In the bundled two-commit push (the default), GitHub still runs CI because the work commit touches non-ignored files alongside the jsonl, which is the correct behavior.
